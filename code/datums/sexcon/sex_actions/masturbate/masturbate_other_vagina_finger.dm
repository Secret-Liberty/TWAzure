...
/datum/sex_action/masturbate_other_vagina_finger/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(span_warning("[user] вводит свои пальцы внутрь лона [target]..."))

/datum/sex_action/masturbate_other_vagina_finger/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	user.visible_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()] [pick("двигает пальцами в лоне", "толкается пальцами в киске", "ласкает пальцами вагину")] [target]..."))
